...
Note: s2 functionality is used by default on geometries with geographic (longitude/latitude) coordinate systems. See [this](https://r-spatial.org/r/2020/06/17/s2.html) for further information.
...
RspatialPkg::get_geom_sf(
  ...
  scale_colors = RColorBrewer::brewer.pal(n = 10, name = "YlOrRd"),
  scale_labels = seq(from=0,to=150000, 15000),
  scale_breaks = seq(from=0,to=150000, 15000)
)
```
...
RspatialPkg::get_geom_raster(
  ...
  scale_breaks = seq(1000, 3000, 250),
  scale_labels = seq(1000, 3000, 250),
...
RspatialPkg::get_geom_raster(
  ...
  legend_key_width = 0.7,
  legend_key_height = 1.0,
  scale_breaks = seq(5000,35000,5000),
  scale_labels = seq(5000,35000,5000),
  hide_x_tics = T,
  hide_y_tics = T
)
```
